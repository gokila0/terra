pipeline {
    ...
    environment {
        // Define image names
        BACKEND_IMAGE = "gokila070/docker-backend:latest"
        FRONTEND_IMAGE = "gokila070/docker-frontend:latest"
    ...
    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'gokila0', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/gokila0/terra.git", branch: 'main'
                }
            }
        }
        ...
        stage('Login to Docker Registry') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'gokila070', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
